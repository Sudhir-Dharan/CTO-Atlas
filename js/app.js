async function loadPartials() {
  ...
  await Promise.all(Array.from(includes).map(async el => {
    const resp = await fetch(el.dataset.include + '?v=1');
...
function setupCharts() {
  ...
  new Chart(document.getElementById('ctoSkillsChart'), {
    type: 'radar',
    data: {
      labels: ['Leadership', 'Strategy', 'Tech Knowledge', 'Execution', 'Communications', 'Innovation'],
      datasets: [{
        label: 'Skill Level',
        data: [8, 9, 7, 8, 8, 9],
        fill: true,
  ...
  new Chart(document.getElementById('techStrategyRadarChart'), {
    type: 'radar',
    data: {
      labels: ['Business Alignment', 'Innovation', 'Governance', 'Ops Efficiency', 'Customer Focus'],
      datasets: [{
        label: 'Maturity',
        data: [7, 8, 6, 7, 8],
        fill: true,
  ...
  new Chart(document.getElementById('facilityCostChart'), {
    ...
    data: {
      labels: ['1', '2', '3', '4', '5'],
      datasets: [
        { label: 'Transportation', data: [5, 6, 7, 8, 9], borderColor: '#4381b4', fill: false },
        { label: 'Inventory', data: [6, 7, 8, 9, 10], borderColor: '#4eb392', fill: false },
        { label: 'Facility', data: [4, 5, 6, 7, 8], borderColor: '#e63946', fill: false }
      ]
    },
    options: {
      maintainAspectRatio: false,
      plugins: {
        title: { display: true, text: 'Cost Trade-Offs in Facility Network Design', font: { size: 16 } }
      },
      scales: {
    ...
    try {
      ...
      if (result.candidates && result.candidates.length > 0 && result.candidates[0].content && result.candidates[0].content.parts && result.candidates[0].content.parts.length > 0) {
        ...
        const formattedHtml = text
          ...
          .map(line => {
            line = line.trim();
            if (line.startsWith('* ')) return `<li class="text-content">${line.substring(2)}</li>`;
            if (line && !line.startsWith('**')) return `<p class="text-content">${line}</p>`;
            return '';
          })
          .join('')
          .replace('</ul><ul>', '<ul>');
